...
- **Automatic JSON Schema sanitization** for tools and `response_format`:
  - Removes Anthropic-rejected keywords (`$schema`, `$defs`, `$ref`, `exclusiveMinimum`, `pattern`, `format`, `minLength`, etc.).
  - Inlines local `$ref` references and breaks reference cycles safely.
  - Rewrites `oneOf`/`allOf` to `anyOf` or merged properties.
