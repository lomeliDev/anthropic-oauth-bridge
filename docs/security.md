...
|---|---|---|
| **Anthropic API keys** and AWS credentials (= billing on your account) | `.env` (`ANTHROPIC_API_KEY`), `accounts.json` | files written with mode `600`/`700`; never logged; never returned by any endpoint |
...
1. **Client → bridge**: `Authorization: Bearer <api_key>`. In multi-account mode each key maps to one Anthropic account.
...
3. **Bridge → Anthropic**: short-lived access tokens refreshed from the refresh token; scopes limited to what the CLI uses
...
request internal services: `http://127.0.0.1:<port>/admin/...` (this bridge), `169.254.169.254`
...
- This uses a consumer subscription (Anthropic AI Pro/Ultra) through the Anthropic Bridge CLI's OAuth client. It is the same
  traffic the CLI generates, minus the CLI system prompt. Quota, rate limits and ToS are Anthropic's.
- The User-Agent/`cl` fingerprint is configurable (`AGY_CLI_VERSION`, `AGY_CLI_CL`) — keep it in sync with the real
  CLI if Anthropic starts rejecting old ones (`docs/reverse-engineering.md`).
